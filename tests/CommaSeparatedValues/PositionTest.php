...
describe('CommaSeparatedValues position management', function () {
    describe('withBom() and getEncoding()', function () {
        describe('with BOM file', function () {
            beforeEach()->with([fn() => makeCsv(__DIR__ . '/fixtures/utf8-bom.csv')]);

            test('restores cursor position on BOM file (called from position 0)', function (CommaSeparatedValues $csv) {
            ...
            test('restores cursor position when called from a non-zero position', function (CommaSeparatedValues $csv) {
            ...
            test('cached call does not change cursor position', function (CommaSeparatedValues $csv) {
            ...
            test('does not interfere with subsequent readRow()', function (CommaSeparatedValues $csv) {
                $csv->withBom();
                $csv->getEncoding();
            ...
            test('multiple calls before readRow() do not affect row count', function (CommaSeparatedValues $csv) {
                $csv->withBom();
                $csv->withBom();
                $csv->getEncoding();
                $csv->getEncoding();
        ...
        describe('with non-BOM file', function () {
            beforeEach()->with([fn() => makeCsv(__DIR__ . '/fixtures/utf8.csv')]);

            test('restores cursor position on non-BOM file (called from position 0)', function (CommaSeparatedValues $csv) {
                $positionBefore = $csv->ftell();
                $csv->withBom();
                expect($csv->ftell())->toBe($positionBefore);
    ...
    describe('readRow()', function () {
        ...
        test('cursor position is unchanged between yields during iteration and full iteration', function (CommaSeparatedValues $csv) {
            $positionBefore = $csv->ftell();
        ...
        test('cursor position is restored after early break', function (CommaSeparatedValues $csv) {
            $positionBefore = $csv->ftell();
        ...
        test('two concurrent generators do not interfere with each other', function (CommaSeparatedValues $csv) {
        ...
        test('multiple sequential readRow() calls each return all rows', function (CommaSeparatedValues $csv) {
        ...
        test('finally clears recorded positions when generator is abandoned via unset()', function (CommaSeparatedValues $csv) {
            $positionBefore = $csv->ftell();
