...
describe('CommaSeparatedValues for UTF-8 BOM csv', function () {
    ...
    test('getBasename returns correct basename', function (CommaSeparatedValues $csv) {
        expect($csv->getBasename())->toBe(pathinfo(__DIR__ . '/fixtures/utf8-bom.csv', PATHINFO_BASENAME));
    });

    test('withBom returns true', function (CommaSeparatedValues $csv) {
        expect($csv->withBom())->toBeTrue();
    });
